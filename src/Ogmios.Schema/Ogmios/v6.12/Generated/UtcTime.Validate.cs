...
public readonly partial struct UtcTime
{
    ...
    public ValidationContext Validate(in ValidationContext validationContext, ValidationLevel level = ValidationLevel.Flag)
    {
        ...
        if (level > ValidationLevel.Basic)
        {
            ...
            result = result.PushSchemaLocation("https://endjin.com/Users/davebeaumont/source/cardano-public/OgmiosDotnet/src/Ogmios.Schema/Ogmios/v6.12/Source/cardano.json#/definitions/UtcTime");
        }
            ...
            ValidationLevel level = ValidationLevel.Flag)
        {
            if (valueKind != JsonValueKind.String)
            {
                if (level == ValidationLevel.Verbose)
                {
                    ...
                    ignoredResult = ignoredResult.WithResult(isValid: true, $"Validation format - ignored 'date-time' because the value is of kind '{valueKind}' not 'String'.");
            ...
            return Corvus.Json.ValidateWithoutCoreType.TypeDateTime(value, validationContext, level, "format");
        }
            ...
            ValidationLevel level = ValidationLevel.Flag)
        {
            ...
            static bool StringValidator(ReadOnlySpan<char> input, in Corvus.Json.Validate.ValidationContextWrapper context, out ValidationContext result)
            {
                ...
                if (Pattern.IsMatch(input))
                {
                    ...
                    {
                        result = result.WithResult(isValid: true, validationLocationReducedPathModifier: new JsonReference("pattern"), $"Validation pattern - {input.ToString()} matched '[0-9]{{4}}-[0-9]{{2}}-[0-9]{{2}}T[0-9]{{2}}:[0-9]{{2}}:[0-9]{{2}}.?[0-9]*Z?'");
                    }
                }
                else
                {
                    ...
                    {
                        result = result.WithResult(isValid: false, validationLocationReducedPathModifier: new JsonReference("pattern"), $"Validation pattern - {input.ToString()} did not match '[0-9]{{4}}-[0-9]{{2}}-[0-9]{{2}}T[0-9]{{2}}:[0-9]{{2}}:[0-9]{{2}}.?[0-9]*Z?'");
                    }
                    else
                    {
                        result = result.WithResult(isValid: false, validationLocationReducedPathModifier: new JsonReference("pattern"), "Validation pattern - The value did not match '[0-9]{{4}}-[0-9]{{2}}-[0-9]{{2}}T[0-9]{{2}}:[0-9]{{2}}:[0-9]{{2}}.?[0-9]*Z?'");
                    }
                }
