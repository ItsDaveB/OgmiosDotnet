...
public readonly partial struct StakeAddress
{
    ...
    public ValidationContext Validate(in ValidationContext validationContext, ValidationLevel level = ValidationLevel.Flag)
    {
        ...
        if (level > ValidationLevel.Basic)
        {
            ...
            result = result.PushSchemaLocation("https://endjin.com/Users/davebeaumont/source/cardano-public/OgmiosDotnet/src/Ogmios.Schema/Ogmios/v6.10/Source/cardano.json#/definitions/StakeAddress");
        }
            ...
            ValidationLevel level = ValidationLevel.Flag)
        {
            ...
            static bool StringValidator(ReadOnlySpan<char> input, in Corvus.Json.Validate.ValidationContextWrapper context, out ValidationContext result)
            {
                ...
                if (Pattern.IsMatch(input))
                {
                    ...
                    {
                        result = result.WithResult(isValid: true, validationLocationReducedPathModifier: new JsonReference("pattern"), $"Validation pattern - {input.ToString()} matched '^(stake|stake_test)1[0-9a-z]*$'");
                    }
                }
                else
                {
                    ...
                    {
                        result = result.WithResult(isValid: false, validationLocationReducedPathModifier: new JsonReference("pattern"), $"Validation pattern - {input.ToString()} did not match '^(stake|stake_test)1[0-9a-z]*$'");
                    }
                    else
                    {
                        result = result.WithResult(isValid: false, validationLocationReducedPathModifier: new JsonReference("pattern"), "Validation pattern - The value did not match '^(stake|stake_test)1[0-9a-z]*$'");
                    }
                }
