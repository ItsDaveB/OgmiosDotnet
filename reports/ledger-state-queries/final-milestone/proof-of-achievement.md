# Final Milestone - Proof of Achievement

## Evidence of Milestone Completion

| Evidence               | Link                                                            |
| ---------------------- | --------------------------------------------------------------- |
| GitHub Repository      | https://github.com/ItsDaveB/OgmiosDotnet                        |
| NuGet Package          | https://www.nuget.org/packages/OgmiosDotnetClient.Services      |
| GitHub Release         | https://github.com/ItsDaveB/OgmiosDotnet/releases/tag/v6.13.1.3 |
| Test Report            | [test-report.md](./test-report.md)                              |
| Final Closeout Video   | https://youtu.be/BRPxhRNRwJc                                    |
| Final Close-Out Report | [close-out-report.md](./close-out-report.md)                    |
| Documentation (README) | https://github.com/ItsDaveB/OgmiosDotnet/blob/main/README.md    |

---

## Acceptance Criteria

### 1. NuGet Package Published

- **Package**: https://www.nuget.org/packages/OgmiosDotnetClient.Services
- **Version**: 6.13.1.3

### 2. GitHub Release Published

- **Release**: https://github.com/ItsDaveB/OgmiosDotnet/releases/tag/v6.13.1.3

### 3. Documentation Complete

- **Main README**: https://github.com/ItsDaveB/OgmiosDotnet/blob/main/README.md
- **Ledger State Queries**: https://github.com/ItsDaveB/OgmiosDotnet/blob/main/src/Ogmios.Services/LedgerStateQueries/docs/README.md

### 4. All Tests Pass

- **Test Report**: [test-report.md](./test-report.md)

### 5. Final Close-Out Report

- **Report**: [close-out-report.md](./close-out-report.md)

### 6. Final Closeout Video

- **Video**: https://youtu.be/BRPxhRNRwJc
- **Details**: [demo-videos.md](./demo-videos.md)
